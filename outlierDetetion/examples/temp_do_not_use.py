...
from odtoolkit.utils.stat_models import pairwise_distances_no_broadcast

from odtoolkit.models.base import BaseDetector
...
class odtoolkitDataset(torch.utils.data.Dataset):
    def __init__(self, X, y=None, mean=None, std=None):
        super(odtoolkitDataset, self).__init__()
        self.X = X
        self.mean = mean
        self.std = std
...
train_set = odtoolkitDataset(X=X_train, mean=mean, std=std)
test_set = odtoolkitDataset(X=X_train, mean=mean, std=std)
...
class AutoEncoder(BaseDetector):
    ...
    def fit(self, X, y=None):
        ...
        train_set = odtoolkitDataset(X=X_train, mean=self.mean, std=self.std)
        train_loader = torch.utils.data.DataLoader(train_set,
                                                   batch_size=batch_size,
                                                   shuffle=True)
    ...
    def decision_function(self, X):
        ...
        dataset = odtoolkitDataset(X=X, mean=self.mean, std=self.std)
